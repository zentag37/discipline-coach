...
function OnboardingPage() {
    ...
    (async () => {
      ...
      if (data) {
        setP({
          ...
          risk_per_trade: data.risk_per_trade != null ? `${data.risk_per_trade}%` : "1%",
          daily_loss_limit: data.daily_loss_limit != null ? `${data.daily_loss_limit}%` : "3%",
          max_trades: data.max_trades != null ? String(data.max_trades) : "3",
  ...
  const finish = async () => {
    ...
    const { error } = await supabase.from("profiles").upsert({
      ...
      risk_per_trade: parseFloat(String(p.risk_per_trade).replace("%", "")) || null,
      daily_loss_limit: parseFloat(String(p.daily_loss_limit).replace("%", "")) || null,
      max_trades: parseInt(String(p.max_trades), 10) || null,
